...
namespace PetVaccinationTrackerSystem_Project
{
    public partial class petProfilePanelVet : UserControl
    {
        ...
        private void PetProfilePanelVPetInfoCard_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void PetProfilePanelVButtonSave_Click(object sender, EventArgs e)
        {
            ...
            var pet = new Pet
            {
                ...
                ColorAndMarkings = txtColorMarkings.Text,
                OwnerName = txtOwnerName.Text,
                OwnerPhoneNumber = int.Parse(txtcontact.Text),
                Notes = txtNotes.Text,
                Image = petpicture.Image != null ? Convert.ToBase64String((byte[])new ImageConverter().ConvertTo(petpicture.Image, typeof(byte[]))) : null,
                UserID = 3 // Assuming UserID is set to 3 for the current user(para ma execute and ma save sa db yung record)
        ...
        private void txtColorMarkings_TextChanged(object sender, EventArgs e)
        {
